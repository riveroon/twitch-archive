...
pub async fn create_new_file(path: &path::Path) -> io::Result<Option<fs::File>> {
    ...
    return file.map(|x| Some(x))
        .or_else(|e| if e.kind() == io::ErrorKind::AlreadyExists { Ok(None) } else { Err(e) });
}
...
pub async fn create_dedup_file(path: &path::Path) -> io::Result<(Box<path::Path>, fs::File)> {
    ...
    if let Some(x) = path.parent() { fs::create_dir_all(x).await? }

    if let Some(x) = create_new_file(path).await? { return Ok((path.into(), x)) };
    ...
    for i in 1..MAX_FILENAME_DUP {
        ...
        if let Some(x) = create_new_file(&new_path).await? { return Ok((new_path.into(), x)) };
    }
...
pub async fn create_new_dir(path: &path::Path) -> io::Result<bool> {
    ...
    fs::create_dir(path).await
        .map(|_| true)
        .or_else(|e| if e.kind() == io::ErrorKind::AlreadyExists { Ok(false) } else { Err(e) })
}
...
pub async fn create_dedup_dir(path: &path::Path) -> io::Result<Box<path::Path>> {
    ...
    if create_new_dir(path).await? { return Ok(path.into()) }

    for i in 1..MAX_FILENAME_DUP {
        let mut new_name = path.file_name()
            .unwrap_or_default()
            .to_os_string();
        new_name.push(format!("-{}", i));
        let new_path = path.with_file_name(&new_name);

        if create_new_dir(&new_path).await? { return Ok(new_path.into_boxed_path()) }
    }
    return Err(io::ErrorKind::AlreadyExists.into());
}