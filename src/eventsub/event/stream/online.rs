...
impl SubscriptionType for Online {
    ...
    fn name() -> &'static str { "stream.online" }
    fn ver() -> &'static str { "1" }
}
...
impl OnlineCond {
    pub fn from_id(id: impl ToString) -> Self { OnlineCond { user_id: id.to_string().into() } }
}

impl From<&User> for OnlineCond {
    fn from(value: &User) -> Self {
        return Self::from_id(value.id());
    }
}
...
use crate::{helix::{User, Stream, get_streams, StreamFilter}, HelixAuth};
impl OnlineEvent {
    ...
    pub async fn to_stream(self, auth: HelixAuth) -> surf::Result<Option<Stream>> {
        return get_streams(auth, std::iter::once(
            StreamFilter::User( &self.user )
        )).try_next().await;
    }
}
